...
export function recalcPlayerDerivedStats(player: Player): void {
    ...
    const maxHealth = calcHealth(level, player.STR, player.VIT, player.DEX, player.WIS);
    const maxMana = calcMana(level, player.INT, player.WIS);
    const defense = calcDefense(level, player.STR, player.VIT, player.DEX);
    const resistance = calcResistance(level, player.INT, player.WIS, player.LUK);

    // Preserve current health/mana proportionally, clamped to new max
    const healthRatio = player.maxHealth > 0 ? player.currHealth / player.maxHealth : 1;
    const manaRatio = player.maxMana > 0 ? player.currMana / player.maxMana : 1;

    player.maxHealth = maxHealth;
    player.currHealth = Math.min(maxHealth, Math.max(0, Math.round(maxHealth * healthRatio)));

    player.maxMana = maxMana;
    player.currMana = Math.min(maxMana, Math.max(0, Math.round(maxMana * manaRatio)));

    player.defense = Math.max(defense, 0);
    player.resistance = Math.max(resistance, 0);
}

function calcHealth(level: number, STR: number, VIT: number, DEX: number, WIS: number): number {
    return (
...
function calcMana(level: number, INT: number, WIS: number): number {
    return (
...
function calcDefense(level: number, STR: number, VIT: number, DEX: number): number {
    return (
...
function calcResistance(level: number, INT: number, WIS: number, LUK: number): number {
    return (
