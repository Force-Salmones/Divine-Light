import { serverGameState, type Player, type Enemy } from "./gamestate";
import type { Request, Response, NextFunction } from "express";
import { makeGameStateSnapshot } from "./makeSnapshot";
import { PLAYER_SIZE } from "../constants.js";

export async function handlerMovePlayer(
	req: Request,
	res: Response,
	next: NextFunction,
) {
	const { x, y, enemyId } = req.body as {
		x?: number;
		y?: number;
		enemyId?: number;
	};
	...
	if (typeof enemyId === "number") {
		...
		if (distance <= player.attackRange) {
			...
			return res.json({
				success: true,
				message: "Already in attack range",
				gameState: makeGameStateSnapshot(resolvedPlayerId),
			});
		}
		...
		const desiredDistance = Math.max(
			0,
			player.attackRange - approachMargin,
		);
		if (distance === 0) {
	...
	return res
		.status(400)
		.json({ success: false, message: "Invalid move request" });
}
